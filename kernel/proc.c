...
void updateactiveprocnum(int delta) {
...
found:
  ...
  if ((p->roregion = (struct usyscall *)kalloc()) == 0) {
    freeproc(p);
    release(&p->lock);
    return 0;
  }
...
uint64 getusedproc(void) { return usedprocnum; }
...
static void freeproc(struct proc *p) {
  ...
  p->trapframe = 0;
  if (p->roregion) kfree((void *)p->roregion);
  p->roregion = 0;
...
pagetable_t proc_pagetable(struct proc *p) {
  ...
  if (mappages(pagetable, USYSCALL, PGSIZE, (uint64)(p->roregion),
               PTE_R | PTE_U) < 0) {
    uvmunmap(pagetable, TRAMPOLINE, 1, 0);
